...
Shared-host test note: e2e tests start the server in-process on an ephemeral port (`test/e2e/harness.ts` `startServer()`, port 0), so `bun test` never conflicts with sibling agents' test runs or a dev server occupying 4566/5666. New e2e files must use `startServer()` from the harness — do not spawn a server subprocess or hardcode ports. Validate your own change by running your service's e2e file, then rely on GitHub CI as the authoritative signal. Once your own test passes and CI is green, move straight to `Merging`.
