...
const CEDAR_EXACT_RE =
  /^\s*(permit|forbid)\s*\(\s*principal\s*==\s*([A-Za-z][A-Za-z0-9_]*(?:::[A-Za-z][A-Za-z0-9_]*)*::"[^"]*")\s*,\s*action\s*==\s*([A-Za-z][A-Za-z0-9_]*(?:::[A-Za-z][A-Za-z0-9_]*)*::"[^"]*")\s*,\s*resource\s*==\s*([A-Za-z][A-Za-z0-9_]*(?:::[A-Za-z][A-Za-z0-9_]*)*::"[^"]*")\s*\)\s*;\s*$/;

const parseExactMatchPolicy = (statement: string): ParsedCedarPolicy | null => {
  const m = statement.trim().match(CEDAR_EXACT_RE);
  if (!m) return null;
  const principal = parseEntityId(m[2]);
  const actionEntity = parseEntityId(m[3]);
  const resource = parseEntityId(m[4]);
  if (!principal || !actionEntity || !resource) return null;
  return {
    effect: m[1] as "permit" | "forbid",
    principal,
    action: {
      actionType: actionEntity.entityType,
      actionId: actionEntity.entityId,
    },
    resource,
  };
};
...
const DeletePolicyStore: OperationHandler = (input, ctx) => {
  const rawId = requireString(input, "policyStoreId");
  const store = requireStore(ctx, rawId);
  ctx.store.delete(storeKey(store.policyStoreId));
...
const CreatePolicy: OperationHandler = (input, ctx) => {
  ...
  if (staticDef) {
    ...
    if (statement) {
      parsed = parseExactMatchPolicy(statement);
    }
  } else if (tlDef) {
...
const TagResource: OperationHandler = (input, ctx) => {
  ...
  const storeId = resourceArn.split("/").pop();
  if (storeId) {
    const store = ctx.store.get<StoredPolicyStore>(storeKey(storeId));
    if (store) {
      ctx.store.set(storeKey(storeId), {
        ...store,
...
const UntagResource: OperationHandler = (input, ctx) => {
  ...
  if (storeId) {
    ...
    if (store) {
      ...
      ctx.store.set(storeKey(storeId), { ...store, tags: storeTags });
    }
  }
